...
core.main = function(fileName, program) {
  ...
  request(
    ...
    function(error, response, body) {
      ...
      pthis.params.token = body.token;
      pthis.params.unique = body.unique;

      var permalink = "http://ca.storyboards.jp/a/" + pthis.params.unique;
      ...
      var castFileName = fileName;
      var splitFileName = fileName.match(".+/(.+?)$");
      if (splitFileName) {
        castFileName = splitFileName[1];
      }

      var params = {
        method: 'PUT',
        uri: 'http://ca.storyboards.jp/api/storyboards/-/castoapi/code/' + pthis.params.unique,
        form: {
          body: '',
          token: pthis.params.token,
          filename: castFileName,
          unique: pthis.params.unique
        }
      };
